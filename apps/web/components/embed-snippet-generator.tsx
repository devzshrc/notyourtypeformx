...
export function EmbedSnippetGenerator({ formId, baseUrl = typeof window !== "undefined" ? window.location.origin : "" }: Props) {
    ...
    const iframeSnippet = `<iframe src="${baseUrl}/form/${formId}?embed=1" width="100%" height="600" frameborder="0" style="border:none;"></iframe>`;
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => copy(iframeSnippet, "iframe")}>
                        {copied === "iframe" ? <CheckCircle className="size-3.5 text-foreground" /> : <Copy className="size-3.5" />}
            ...
            <div className="space-y-2">
                <div className="flex items-center justify-between">
                    ...
                    <Button variant="ghost" size="sm" onClick={() => copy(scriptSnippet, "script")}>
                        {copied === "script" ? <CheckCircle className="size-3.5 text-foreground" /> : <Copy className="size-3.5" />}
